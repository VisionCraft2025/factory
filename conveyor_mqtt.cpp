...
int main(int argc, char *argv[]) {
    ...
    QObject::connect(&mqtt_client, &QMqttClient::connected, [&](){
        ...
        // 토픽 구독
        auto subscription = mqtt_client.subscribe(QMqttTopicFilter("conveyor/cmd"));
        if (subscription) {
            qDebug() << "conveyor/cmd 토픽 구독 요청 전송";
        ...
        QTimer::singleShot(2000, [&]() {
            qDebug() << ">> 테스트 메시지 발행 중...";
            mqtt_client.publish(QMqttTopicName("conveyor/cmd"), "test_message");
        });
    });
    ...
    qDebug() << "MQTT 클라이언트 시작 - conveyor/cmd 토픽 대기 중...";
