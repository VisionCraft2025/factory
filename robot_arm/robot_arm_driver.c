...
static int angle_to_pwm_us(int angle)
{
    // 각도가 0도나 180도 이상 못 넘어가도록 제한
    if (angle < 0)
        angle = 0;
    if (angle > 180)
        angle = 180;

    // 서보모터는 각도로 못 알아들으므로, 모터를 돌리는 시간을 컨트롤해서 각도를 조절해야 함
    // 1000ms: 0도, 1500ms: 90도, 2000ms: 180도 -> 이건 서보모터 만든 제조사에서 정한거임
    return PWM_MIN_US + (angle * (PWM_MAX_US - PWM_MIN_US)) / 180;
}

// // 특정 서보모터에 PWM 신호를 주는 함수
// static void set_servo_pwm(int servo_id, int angle)
// {
//     static int last_angles[NUM_SERVOS] = { -1, -1, -1, -1 };
    
//     if (last_angles[servo_id] == -1 || abs(last_angles[servo_id] - angle) >= DEADBAND_DEGREES)
//     {
//         last_angles[servo_id] = angle;

//         // 각도를 펄스폭(시간)으로 변환 -> 시간이 길면 길수록 모터가 돌아가는 시간 길어짐 -> 더 많은 각도로 돌아감
//         int pulse_width_us = angle_to_pwm_us(angle);
...
static void set_servo_pwm(int servo_id, int angle)
{
    ...
    if (abs(last_angles[servo_id] - angle) >= DEADBAND_DEGREES)
    {
        ...
        int pulse_width_us = angle_to_pwm_us(angle);
        gpio_set_value(servo_pins[servo_id], 1);
        usleep_range(pulse_width_us, pulse_width_us + 10);
        gpio_set_value(servo_pins[servo_id], 0);
    }
}
...
static int robot_motion_thread(void *data)
{
    ...
    while (!kthread_should_stop())
    {
        ...
        if (auto_mode)
        {
            ...
            {
            case 0:                           // natural_A -> 초기위치에서 A지점으로 이동
                move_servo_smooth(1, 75, 8);  // 중단모터를 75도
                move_servo_smooth(2, 100, 8); // 상단모터를 100도
                move_servo_smooth(0, 30, 8);  // 하단모터를 30도
                msleep(4000);                 // 일단 4초동안 대기
                current_sequence = 1;         // 다음 단계로
                break;

            case 1:                          // A_natural -> A지점에서 초기위치로 복귀
                move_servo_smooth(0, 90, 8); // 하단모터를 90도로 (중앙)
                move_servo_smooth(2, 90, 8); // 상단모터를 90도로
                move_servo_smooth(1, 90, 8); // 중단모터를 90도로
                current_sequence = 2;        // 다음 단계로
                break;

            case 2:                           // natural_B -> 초기위치에서 B지점으로 이동하고 그리퍼 동작해서 물건 놓기
                move_servo_smooth(1, 50, 8);  // 중단모터를 50도로
                move_servo_smooth(2, 120, 8); // 상단모터를 120도로
                move_servo_smooth(0, 145, 8); // 하단모터를 145도로
                msleep(200);                  // 잠시 대기
                move_servo_smooth(3, 180, 8); // 그리퍼 180도로 열기
                msleep(500);                  // 0.5초 대기
                move_servo_smooth(3, 90, 8);  // 그리퍼 90도로 닫기
                current_sequence = 3;         // 다음 단계로
                break;

            case 3:                          // B_natural -> B지점에서 초기위치로 복귀
                move_servo_smooth(0, 90, 8); // 하단모터를 90도로
                move_servo_smooth(1, 90, 8); // 중단모터를 90도로
                move_servo_smooth(2, 90, 8); // 상단모터를 90도로
                msleep(1000);                // 1초 대기
                current_sequence = 0;        // 처음부터 다시 -> 무한 반복
                break;
            }
        }
        else
...
static ssize_t robot_arm_write(struct file *file, const char __user *buf, size_t len, loff_t *offset)
{
    ...
    else if (sscanf(command_buffer, "servo%d %d", &servo_id, &angle) == 2)
    {
        ...
        if (servo_id >= 0 && servo_id < NUM_SERVOS && angle >= 0 && angle <= 180)
